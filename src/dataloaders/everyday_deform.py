...
class EverydayDeformDataset(Dataset):
    ...
    def __init__(self, object_classes, root_dir):
        ...
        for obj_class in object_classes:
            ...
            soft_rest_files = [f for f in os.listdir(os.path.join(root_dir,'deformations', obj_class)) if f.endswith('_rest.obj')]

            for soft_rest_file in soft_rest_files:
                # Get the sample id by removing the '_rest.obj' suffix
                sample_id = soft_rest_file[:-9]
    ...
    def __getitem__(self, idx):
        ...
        soft_rest_mesh = o3d.io.read_triangle_mesh(os.path.join(self.root_dir,'deformations', obj_class, sample_id + "_rest.obj"))
        soft_def_mesh = o3d.io.read_triangle_mesh(os.path.join(self.root_dir,'deformations', obj_class, sample_id + "_def.obj"))

        soft_rest_mesh_t = torch.tensor(np.asarray(soft_rest_mesh.vertices), dtype=torch.float32)
        soft_def_mesh_t = torch.tensor(np.asarray(soft_def_mesh.vertices), dtype=torch.float32)
        ...
        return soft_rest_mesh_t, soft_def_mesh_t, meta_data
