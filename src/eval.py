        ...
        for batch_idx, (obj_name, soft_rest_graphs, soft_def_graphs, meta_data, rigid_graphs) in enumerate(dataloader_val):
            ...
            if visualize:
                for indx in range(config.dataloader.batch_size):
                    ...
                    rigid_mesh_path = os.path.join('./outputs/', meta_data['sample_path'][indx] + '_rigid.obj')
                    o3d.io.write_triangle_mesh(rigid_mesh_path, rigid_mesh)

                    soft_mesh = meta_data['soft_rest_mesh'][indx]

                    # For resting
                    soft_mesh.vertices = o3d.utility.Vector3dVector(soft_rest_graphs[indx].pos.cpu().numpy())
                    resting_mesh_path = os.path.join('./outputs/', meta_data['sample_path'][indx] + '_resting.obj')
                    o3d.io.write_triangle_mesh(resting_mesh_path, soft_mesh)

                    # For gt
                    soft_mesh.vertices = o3d.utility.Vector3dVector(soft_def_graphs_batched[indx].pos.cpu().numpy())
                    gt_mesh_path = os.path.join('./outputs/', meta_data['sample_path'][indx] + '_gt.obj')
                    o3d.io.write_triangle_mesh(gt_mesh_path, soft_mesh)

                    # For prediction
                    soft_mesh.vertices = o3d.utility.Vector3dVector(predictions[indx].pos.cpu().numpy())
                    pred_mesh_path = os.path.join('./outputs/', meta_data['sample_path'][indx] + '_pred.obj')
